...
cross_regression = function(type1, var2, group1, group2, winsorize1, year1, risk1){
  ...
  ### Cluster SE
  m1 = extract_coeftest(coeftest(m1, cluster.vcov(m1, dta_reg$list_id, df_correction = T)), 0) %>% add_column(.after = 'var', N = (x1))
...
dta_outcomes$consult_care_plan
...
count = 0
# for(var1 in c('consult_care_plan', 'consult_gp', 'consult_nurse', 'consult_any')){
# for(var1 in c('n_inpatient', 'n_outpatient', 'n_primary', 'n_diag', 'n_diag_av', 'readmit_30_any')){
# for(var1 in c('copd_any', 'heart_failure_any', 'myocardial_infarction_any', 'pneumonia_any', 'stroke_any', 'n_sever_diag_any')){
for(var1 in c('p_a_diabetes', 'p_c_beta_blockers', 'p_statins', 'p_c_hypertensive', 'p_key', 'p_other')){
# for(var1 in c('m_glicihem_riigi', 'm_creatinine_riigi', 'm_chol_tri_riigi',  'm_glucose_riigi', 'm_ecg_riigi', 'm_thc_riigi')){
    ...
    # fwrite(dta_table, file.path(project_path,  'Tables', 'table2_cross_temp.csv'))
    
}
  
dta_deaths_save = dta_table
